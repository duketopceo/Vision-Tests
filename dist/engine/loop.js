...
export class Engine {
    ...
    _note(stage, message, context) {
        const rec = { stage, message, ...(context !== undefined ? { context } : {}) };
     ...
     */
    async locate(instruction, cached) {
    ...
    async _locateWithModel(instruction, observation, useHeal, modelOverride) {
        ...
        try {
            ...
            this._note('locate', 'model call threw', e.message);
            throw e;
        }
        if (!response) {
            return {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            const coordsOk = action.action === 'click' &&
                typeof action.x === 'number' &&
                Number.isFinite(action.x) &&
                typeof action.y === 'number' &&
                Number.isFinite(action.y);
            const probe = coordsOk ? await this._resolveNode(action.x, action.y) : null;
            ...
            if (attempt === 1 || !this._opts.ledger.canSpend(0.001))
                break;
            this._note('locate', action.action !== 'click'
                ...
                : coordsOk
                    ...
                    : 'grounding corrected after missing/invalid coords', `attempt=${attempt} instruction=${instruction.slice(0, 80)}`);
    ...
    async _callModel(kind, messages, opts = {}) {
        ...
        if (this._opts.config.budgetUsd !== undefined &&
    ...
    _parseAction(content) {
        try {
            ...
            const variantKey = ['click', 'type', 'pressKeys', 'scroll', 'wait', 'done', 'fail'].find((k) => k in parsed);
            if (parsed.action === undefined && variantKey !== undefined) {
                ...
                if (typeof v === 'object' && v !== null)
                    ...
                    if (typeof out.x === 'string')
                        out.x = Number(out.x);
                    if (typeof out.y === 'string')
                        out.y = Number(out.y);
                }
            ...
            return { action: 'fail', reasoning: `JSON parse failed: ${e.message}` };
        }
    }
    _parseAssertion(content) {
        try {
            const parsed = JSON.parse(content);
            ...
            return { verdict: 'fail', reasoning: `JSON parse failed: ${e.message}` };
        }
    }
    async _resolveAction(action) {
        if (typeof action.x !== 'number' || !Number.isFinite(action.x) || typeof action.y !== 'number' || !Number.isFinite(action.y)) {
            return undefined;
        }
        return this._resolveNode(action.x, action.y);
    }
    async _resolveNode(x, y) {
        const info = await this._opts.driver.rawPage.evaluate(([cx, cy]) => {
            ...
            const snippet = (el.getAttribute('aria-label') || el.textContent || '')
                .trim()
                .slice(0, 200);
            return {
                x: Math.round(rect.x),
    ...
    async _buildFingerprint(instruction, action, resolved, model) {
